...
output[,'lower95'] = c(qbeta(.025, a+y, b+n-y), qtrunc(.025, spec="beta", a=, b=b, shape1=(c+y), shape2=(d+n-y)))
output[,'upper95'] = c(qbeta(.975, a+y, b+n-y), qtrunc(.975, spec="beta", a=a, b=b, shape1=(c+y), shape2=(d+n-y)))

print(output)

xtrunc = seq(.1, .029, by=.001) 
ytrunc = truncpost[xtrunc*1000]

xreg = seq(.031, .479, by=.001)
...
polygon( c(xtrunc, rev(xtrunc)), c(ytrunc, rep(0,length(ytrunc))), col=rgb(255,0,0,100,maxColorValue=255), border=NULL)
polygon( c(xreg, rev(xreg)), c(yreg, rep(0,length(yreg))), col=rgb(0,0,255,100,maxColorValue=255), border=NULL)
dev.off()
