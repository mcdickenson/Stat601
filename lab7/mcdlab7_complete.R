...
for(k in 2:M){
  ...
  sigmai0= ( (tau[k] + tau0[k]*T) / (tau[k] * tau0[k]) )^(-1/2)
  for(i in 1:N){
    mui0[i] =  (mu0[k]/tau0[k]) + (sum(Y[i,])-beta1[k-1,i]*sum(X))/tau[k]
  }
  ...
  for(i in 1:N){
    mui1[i] = ( (beta0[k, i]*sum(X) + sum(X*Y[i,]))/tau[k] + mu1[k]/tau1[k])
  }
...
mu1postburn <- mu1[-(1:1000)]
tau1postburn <- tau1[-(1:1000)]
beta1pred <- rnorm(4000, mu1postburn, sqrt(tau1postburn))
mean(beta1pred>.5)
